...
def main():
    ...
    for i in range(n_steps):
        ...
        if i == 50:
            env.add_target_ghost(box.name(), np.array([3,2,math.pi/2]))

        if i == 190:
            env.add_target_ghost(sphere.name(), np.array([-7,2,0]))

        if i == 150:
            env.add_target_ghost(cylinder.name(), np.array([-7,2,math.pi/2]))
