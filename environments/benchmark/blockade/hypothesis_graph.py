...
def main(stage):
    ...
    hgraph.addStartNode(ObstacleNode(2, "R: start cube", []))

    if stage==1:

        node1 = ObstacleNode(1, "start robot", [])
        hgraph.current_node = node1
        hgraph.addStartNode(node1)
        hgraph.addNode(ObstacleNode(4, "RM: cube", []))
    ...
    if stage == 5:
        ...
        node7 = ObstacleNode(7, "RM: green wall", [])
        hgraph.current_node = node7
        hgraph.addNode(node7)
        hgraph.addNode(StateNode(8, "R: green wall", []))
        hgraph.addEdge(Edge("id", 1, 2, "MPC", "controller", True))
