...
def test_occupancy_exceptions():
    ...
    # x_index to small
    with pytest.raises(ValueError):
        occ_map.p_idx_to_occupancy(-1, 0, 0)

    # y_index to large
    with pytest.raises(ValueError):
        occ_map.p_idx_to_occupancy(0, 201, 0)

    # y_index to small
    with pytest.raises(ValueError):
        occ_map.p_idx_to_occupancy(0, -10, 0)

    # n_orientations to large
    with pytest.raises(ValueError):
        occ_map.p_idx_to_occupancy(40, 80, 400)

    # n_orientations to small
    with pytest.raises(ValueError):
        occ_map.p_idx_to_occupancy(40, 40, -40)

    # boolean instead of int
    with pytest.raises(TypeError):
        occ_map.p_idx_to_occupancy(40, 80, True)
...
def test_c_idx_to_cart_2d_out_of_bounds():
    ...
    with pytest.raises(IndexError):
        occ_map.c_idx_to_cart_2d(3, 7)
    with pytest.raises(IndexError):
        occ_map.c_idx_to_cart_2d(0, -1)
    with pytest.raises(IndexError):
...
def test_cart_2d_to_c_idx_in_cell():
    ...
    assert (1, 1) == occ_map.cart_2d_to_c_idx(-14.99, 14.99)
    assert (1, 1) == occ_map.cart_2d_to_c_idx(-14.99, -14.99)
    assert (1, 1) == occ_map.cart_2d_to_c_idx(14.99, 14.99)
    assert (1, 1) == occ_map.cart_2d_to_c_idx(14.99, -14.99)

    assert (0, 2) == occ_map.cart_2d_to_c_idx(-15.01, 15.01)
    assert (0, 2) == occ_map.cart_2d_to_c_idx(-15.01, 44.99,)
    assert (0, 2) == occ_map.cart_2d_to_c_idx(-44.99, 15.01)
...
def test_cart_2d_to_c_idx_on_boundary():
    ...
    assert (0,0) == occ_map.cart_2d_to_c_idx(-12.5, -20)
    assert (0,1) == occ_map.cart_2d_to_c_idx(-12.5, -15)
    assert (5,7) == occ_map.cart_2d_to_c_idx(15, 17.5)
    assert (5,7) == occ_map.cart_2d_to_c_idx(10, 17.5)
    assert (5,7) == occ_map.cart_2d_to_c_idx(12.5, 20)
...
def test_cart_2d_to_c_idx_out_of_bounds():
    ...
    with pytest.raises(IndexError):
        occ_map.cart_2d_to_c_idx(-50.3, 3)
    with pytest.raises(IndexError):
        occ_map.cart_2d_to_c_idx(0, -75.32)
    with pytest.raises(IndexError):
...
def test_position_to_cell_or_grid_edge():
    ...
    assert (5,5) == occ_map.cart_2d_to_c_idx_or_grid_edge(3, 3)
