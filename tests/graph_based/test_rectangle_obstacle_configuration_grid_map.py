...
def test_occupancy_map_arguements():
    ...
    assert 10 == occ_map.obst_x_length
    assert 6 == occ_map.obst_y_length
...
def test_occupancy_exceptions():
    ...
    # x_index to small
    with pytest.raises(ValueError):
        occ_map._p_idx_to_occupancy(-1, 0, 0)

    # y_index to large
    with pytest.raises(ValueError):
        occ_map._p_idx_to_occupancy(0, 201, 0)

    # y_index to small
    with pytest.raises(ValueError):
        occ_map._p_idx_to_occupancy(0, -10, 0)

    # n_orientations to large
    with pytest.raises(ValueError):
        occ_map._p_idx_to_occupancy(40, 80, 400)

    # n_orientations to small
    with pytest.raises(ValueError):
        occ_map._p_idx_to_occupancy(40, 40, -40)
...
def test_c_idx_to_cart_2d_even():
    ...
    occ_map = RectangleObstacleConfigurationGridMap(0.5, 4, 4, {},
            obst_cart_2d=np.array([1,2]),
            obst_name="robot",
            n_orientations=1,
            obst_x_length=2,
            obst_y_length=1)
    assert (-1/4, 1/4) == occ_map._c_idx_to_cart_2d(3, 4)
    assert (-1.75, -1.75) == occ_map._c_idx_to_cart_2d(0, 0)
    assert (3/4, 1/4) == occ_map._c_idx_to_cart_2d(5, 4)

def test_c_idx_to_cart_2d_uneven():
    # test uneven grid_x_length and grid_y_length
    occ_map = RectangleObstacleConfigurationGridMap(1.0, 3, 3, {},
            obst_cart_2d=np.array([1,2]),
            obst_name="robot", 
            n_orientations=1,
            obst_x_length=1,
            obst_y_length=2)
    assert (-1, -1) == occ_map._c_idx_to_cart_2d(0, 0)
    assert (0, 0) == occ_map._c_idx_to_cart_2d(1, 1)
    assert (0, 1) == occ_map._c_idx_to_cart_2d(1, 2)

def test_c_idx_to_cart_2d_out_of_bounds():
    occ_map = RectangleObstacleConfigurationGridMap(1.0, 4, 7, {},
            obst_cart_2d=np.array([1,2]),
            obst_name="robot",
            n_orientations=1,
            obst_x_length=1,
    ...
    with pytest.raises(IndexError):
        occ_map._c_idx_to_cart_2d(4, 3)
    with pytest.raises(IndexError):
        occ_map._c_idx_to_cart_2d(3, 7)
    with pytest.raises(IndexError):
        occ_map._c_idx_to_cart_2d(0, -1)
    with pytest.raises(IndexError):
        occ_map._c_idx_to_cart_2d(-1, 0)

def test_cart_2d_to_c_idx_in_cell():
    occ_map = RectangleObstacleConfigurationGridMap(30, 90, 90, {}, obst_cart_2d=np.array([1,2]), obst_name="robot", n_orientations=1, obst_x_length=1, obst_y_length=1)
    assert (2, 2) == occ_map._cart_2d_to_c_idx(15.01, 15.01)
    assert (2, 2) == occ_map._cart_2d_to_c_idx(44.99, 15.01)

    assert (1, 1) == occ_map._cart_2d_to_c_idx(-14.99, 14.99)
    assert (1, 1) == occ_map._cart_2d_to_c_idx(-14.99, -14.99)
    assert (1, 1) == occ_map._cart_2d_to_c_idx(14.99, 14.99)
    assert (1, 1) == occ_map._cart_2d_to_c_idx(14.99, -14.99)

    assert (0, 2) == occ_map._cart_2d_to_c_idx(-15.01, 15.01)
    assert (0, 2) == occ_map._cart_2d_to_c_idx(-15.01, 44.99,)
    assert (0, 2) == occ_map._cart_2d_to_c_idx(-44.99, 15.01)
    assert (0, 2) == occ_map._cart_2d_to_c_idx(-44.99, 44.99,)

def test_cart_2d_to_c_idx_on_boundary():
    occ_map = RectangleObstacleConfigurationGridMap(5.0, 30, 40, {}, obst_cart_2d=np.array([1,2]), obst_name="robot", n_orientations=4, obst_x_length=1, obst_y_length=10)

    assert (0,0) == occ_map._cart_2d_to_c_idx(-15, -17.5)
    assert (1,0) == occ_map._cart_2d_to_c_idx(-10, -17.5)
    assert (0,0) == occ_map._cart_2d_to_c_idx(-12.5, -20)
    assert (0,1) == occ_map._cart_2d_to_c_idx(-12.5, -15)
    assert (5,7) == occ_map._cart_2d_to_c_idx(15, 17.5)
    assert (5,7) == occ_map._cart_2d_to_c_idx(10, 17.5)
    assert (5,7) == occ_map._cart_2d_to_c_idx(12.5, 20)
    assert (5,7) == occ_map._cart_2d_to_c_idx(12.5, 15)

def test_cart_2d_to_c_idx_out_of_bounds():
    occ_map = RectangleObstacleConfigurationGridMap(1, 100, 150, {}, obst_cart_2d=np.array([1,2]), obst_name="robot", n_orientations=4, obst_x_length=1, obst_y_length=10)
    with pytest.raises(IndexError):
        occ_map._cart_2d_to_c_idx(50.1, 3)
    with pytest.raises(IndexError):
        occ_map._cart_2d_to_c_idx(-50.3, 3)
    with pytest.raises(IndexError):
        occ_map._cart_2d_to_c_idx(0, -75.32)
    with pytest.raises(IndexError):
        occ_map._cart_2d_to_c_idx(-1, 75.01)

def test_position_to_cell_or_grid_edge():
    occ_map = RectangleObstacleConfigurationGridMap(1.0, 6, 6, {}, obst_cart_2d=np.array([1,2]), obst_name="robot", n_orientations=4, obst_x_length=1, obst_y_length=10)

    assert (0,0) == occ_map._cart_2d_to_c_idx_or_grid_edge(-5, -5)
    assert (5,5) == occ_map._cart_2d_to_c_idx_or_grid_edge(5, 5)
    assert (5,5) == occ_map._cart_2d_to_c_idx_or_grid_edge(3, 3)
    assert (3,4) == occ_map._cart_2d_to_c_idx_or_grid_edge(0.1, 1.543)

# def test_occupancy_map_circular_objects():
#     occ_map = RectangleObstacleConfigurationGridMap(1, 5, 4, 2, 1, 1)
#
#     cylinder_dict = {
#             "type": "cylinder",
#             "position": [1.0, 1.0, 1.0],
...
#     occ_map = RectangleObstacleConfigurationGridMap(0.1, 6, 10, 3, 1, 4)
#
#     box_dict = {
#             "type": "box",
#             "position": [1.0, 1.0, 1.0],
...
#     occ_map = RectangleObstacleConfigurationGridMap(1, 5, 6, 1, 2, 8)
#
#     box_dict = {
#             "type": "box",
#             "position": [1.0, 1.0, 1.0],
...
#             "box": box_object}
#
#     occ_map = RectangleObstacleConfigurationGridMap(1.0, 40, 60, objects, obst_cart_2d=np.array([1,2]), n_orientations=1, obst_x_length=1, obst_y_length=5)
#     occ_map.setup()
#
